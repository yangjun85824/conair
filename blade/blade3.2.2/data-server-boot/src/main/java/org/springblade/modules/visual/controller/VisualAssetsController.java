...
@RestController
@AllArgsConstructor
@RequestMapping(LauncherConstant.APPLICATION_VISUAL_NAME + "/assets")
@Tag(name = "可视化资源表", description = "可视化资源表接口")
public class VisualAssetsController extends BladeController {
	...
	public R<VisualAssetsEntity> detail(VisualAssetsEntity visualAssets) {
		VisualAssetsEntity detail = visualAssetsService.getOne(Condition.getQueryWrapper(visualAssets));
	...
	public R<IPage<VisualAssetsEntity>> list(@Parameter(hidden = true) @RequestParam Map<String, Object> visualAssets, Query query) {
		IPage<VisualAssetsEntity> pages = visualAssetsService.page(Condition.getPage(query), Condition.getQueryWrapper(visualAssets, VisualAssetsEntity.class));
		return R.data(pages);
	}
	...
	public R save(@Valid @RequestBody VisualAssetsEntity visualAssets) {
	...
	public R update(@Valid @RequestBody VisualAssetsEntity visualAssets) {
	...
	public R submit(@Valid @RequestBody VisualAssetsEntity visualAssets) {

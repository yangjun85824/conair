...
@Tag(name = "可视化模块", description = "可视化数据接口")
public class VisualController extends BladeController {
	...
	@GetMapping("/list")
	@Parameters({
		@Parameter(name = "title", description = "大屏名称", in = ParameterIn.QUERY, schema = @Schema(type = "string")),
		@Parameter(name = "category", description = "大屏分组", in = ParameterIn.QUERY, schema = @Schema(type = "integer"))
	})
	@ApiOperationSupport(order = 2)
	@Operation(summary = "分页", description = "传入visual")
	public R<IPage<Visual>> list(String title, Integer category, Query query) {
		IPage<Visual> pages = visualService.page(Condition.getPage(query),
			Condition.getQueryWrapper(Kv.create().set("title", title).set("category", category), Visual.class));
		return R.data(pages);
	}

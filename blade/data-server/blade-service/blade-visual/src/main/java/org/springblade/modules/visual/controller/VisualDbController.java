...
@AllArgsConstructor
@RequestMapping("/db")
@Tag(name = "可视化数据源配置模块", description = "可视化数据源配置接口")
public class VisualDbController {
	...
	@Operation(summary = "分页", description = "传入name")
	public R<IPage<VisualDb>> list(String name, Query query) {
		IPage<VisualDb> pages = visualDbService.page(Condition.getPage(query), Condition.getQueryWrapper(Kv.create().set("name", name), VisualDb.class));
		return R.data(pages);
	}

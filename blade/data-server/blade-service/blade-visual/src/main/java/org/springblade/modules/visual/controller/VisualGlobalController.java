...
@RestController
@AllArgsConstructor
@RequestMapping("/visual-global")
@Tag(name = "可视化全局配置模块", description = "可视化全局配置表接口")
public class VisualGlobalController extends BladeController {
	...
	@GetMapping("/list")
	@Parameters({
		@Parameter(name = "globalName", description = "名称", in = ParameterIn.QUERY, schema = @Schema(type = "string"))
	})
	@ApiOperationSupport(order = 2)
	@Operation(summary = "分页", description = "传入globalName")
	public R<IPage<VisualGlobal>> list(String globalName, Query query) {
		IPage<VisualGlobal> pages = visualGlobalService.page(Condition.getPage(query), Condition.getQueryWrapper(Kv.create().set("globalName", globalName), VisualGlobal.class));
		return R.data(pages);
	}

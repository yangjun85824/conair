<template>
  <draggable ghost-class="menu__ghost"
             class="menu_ul"
    ...
    <template #item="{ element }">
      <div :key="element.index"
           v-show="validGroup(element)"
           class="menu__folder"
           :ref="common.NAME+element.index"
           @click.stop="handleSetActive(element,false)"
           @mouseover.stop="mouseOver(element)"
           @mouseout.stop="mouseOut(element)"
           v-if="element.children">
        <div v-contextmenu="{id:contain.menuId,event:contain.handleContextMenu,value:element}"
             :class="['menu__item--folder',{'is-active':handleGetActive(element),'is-over': contain.activeOverIndex===element.index}]">
          <i class="iconfont icon-fold"
             @click.stop="openFolder(element)"
             :class="{'is-active':element.menu}"></i>
          <i class="iconfont icon-folder"
             @click.stop="handleSetActive(element)"></i>
          <input type="text"
          ...
          <span class="menu__menu">
            <el-icon :class="{'is-active':element.auto==true}"
        ...
        <div :key="'list'+element.index"
             class="menu__list"
             v-show="element.menu">
          <layer :count="count+1"
                 :type="type"
                 :key="element.index"
                 :nav="element.children"></layer>
        </div>
      </div>
      <div v-else
           v-show="validGroup(element)"
           :key="element.index"
           v-contextmenu="{id:contain.menuId,event:contain.handleContextMenu,value:element}"
           @click.stop="handleSetActive(element)"
           :class="['menu__item',{'is-active':handleGetActive(element),'is-over': contain.activeOverIndex===element.index}]"
           @mouseover.stop="mouseOver(element)"
           @mouseout.stop="mouseOut(element)">

  ...
  <div class="build">
    ...
    <div class="app"
      ...
      <div class="menu"
        ...
        <div class="menu__drag"
             ref="menuDrag"
             @mousedown="handleDrag($event,'menu')"></div>
      ...
      <div class="menu params"
        ...
        <div class="menu_header">
          ...
          <p class="title"
             v-if="isMain">{{activeObj.title}}（{{activeObj.name}}）</p>
        </div>
        <el-scrollbar class="menu__scrollbar"
                      :style="scrollStyleName">
            ...
            <el-form label-width="110px"
              ...
              <template v-if="isSelectActive">
                ...
                <el-form-item label-width="0">
                  <el-button type="primary"
                             class="block"
                             @click="$refs.contentmenu.handleCompose">成组</el-button>
                </el-form-item>
                <el-form-item label-width="0">
                  <el-button type="danger"
                             class="block"
                             @click="$refs.contentmenu.handleDel">删除</el-button>
                </el-form-item>
                <el-form-item label=""
                              label-width="0px">
                  <monaco-editor v-model="activeObjList"
                                 disabled
              ...
              <template v-else-if="activeIndex">
                ...
                <el-form-item label="隐藏">
                  <avue-switch v-model="activeObj.display"
                               @change="({value})=>handleParams('display', null, value)"></avue-switch>
                </el-form-item>
                <el-form-item label="锁定">
                  <avue-switch v-model="activeObj.lock"
                               @change="({value})=>handleParams('lock', null, value)"></avue-switch>
                </el-form-item>
                <template v-if="isFolder">
                  <el-form-item label="轮播">
                    <avue-switch v-model="activeObj.auto"
                                 @change="({value})=>handleParams('auto', null, value)"></avue-switch>
                  </el-form-item>
                  <el-form-item label="轮播时间"
                                v-if="activeObj.auto">
                    <el-input v-model="activeObj.interval"
                              placeholder="3000">
                      <template #append>
                        <span>毫秒</span>
                      </template>
                  ...
                  <el-form-item label-width="0">
                    <el-button type="primary"
                               class="block"
                               @click="$refs.contentmenu.handleLogout">解散</el-button>
                  </el-form-item>
                  <el-form-item label-width="0">
                    <el-button type="danger"
                               class="block"
                               @click="$refs.contentmenu.handleLogout">删除</el-button>
                  </el-form-item>
                </template>
              ...
              <template v-else>
                <el-tabs class="menu__tabs"
                    ...
                    <el-collapse accordion>
                      ...
                      <el-collapse-item title="滤镜">
                        <el-form-item label="开启">
                          <avue-switch size="small"
                                       v-model="config.styles.show"></avue-switch>
                        </el-form-item>
                        <template v-if="config.styles.show">
                          <el-form-item label="对比度">
                            <avue-slider v-model="config.styles.contrast"
                                         :max="400"></avue-slider>
                          </el-form-item>
                          <el-form-item label="饱和度">
                            <avue-slider v-model="config.styles.saturate"
                                         :max="400"></avue-slider>
                          </el-form-item>
                          <el-form-item label="亮度">
                            <avue-slider v-model="config.styles.brightness"
                                         :max="400"></avue-slider>
                          </el-form-item>
                          <el-form-item label="透明度">
                            <avue-slider v-model="config.styles.opacity"
                                         :max="100"></avue-slider>
                          </el-form-item>
                          <el-form-item label="灰度">
                            <avue-slider v-model="config.styles.grayscale"
                                         :max="400"></avue-slider>
                          </el-form-item>
                          <el-form-item label="色相">
                            <avue-slider v-model="config.styles.hueRotate"
                                         :max="1000"></avue-slider>
                          </el-form-item>
                          <el-form-item label="反转">
                            <avue-slider v-model="config.styles.invert"
                                         :max="400"></avue-slider>
                          </el-form-item>
                          <el-form-item label="模糊">
                            <avue-slider v-model="config.styles.blur"
                                         :max="100"></avue-slider>
                          </el-form-item>
                        </template>
                      </el-collapse-item>
                      <el-collapse-item title="水印">
                        <el-form-item label="开启(预览生效)">
                          <avue-switch v-model="config.mark.show"></avue-switch>
                        </el-form-item>
                        <template v-if="config.mark.show">
                          <el-form-item label="内容">
                            <avue-input v-model="config.mark.text"
                  ...
                  <el-tab-pane label="协同人员"
                               name="5">
                    <div class="sync">
                      <el-button type="primary"
                                 @click="$refs.sync.sync()">同步本地</el-button>
                      <ul>
                        <li v-for="(item,index) in ($refs.sync || {}).userList"
                            :key="index">
                          <img src="https://avuejs.com/images/logo.png"
                               alt="">
                          <el-tooltip class="item"
                                      effect="dark"
                                      placement="top">
                            <template #content>
                              <span>加入时间:{{item.data}}</span>
                            </template>
          ...
          <template v-else-if="menuTabs==1">
            <el-form label-width="120px"
              ...
              <el-form-item label="数据类型">
                <avue-select v-model="activeObj.dataType"
                             placeholder="请选择数据类型"
                             :dic="dicOption.dataType"></avue-select>
              </el-form-item>
              <el-form-item label="映射字段"
                            v-if="validProp('mappingList')">
                <avue-input placeholder="映射字段,默认value"
                            v-model="activeOption.mapping"></avue-input>
              </el-form-item>
              ...
              <template v-else-if="isApi || isNode">
                <el-form-item label="开启跨域"
                  ...
                  <avue-switch v-model="activeObj.proxy"
                               @click="handleRefresh"></avue-switch>
                </el-form-item>
                ...
                <el-form-item label="地址">
                  <avue-input v-model="activeObj.url"
              ...
              <template v-else-if="isRecord">
                <el-form-item label="数据集选择">
                  <avue-select :dic="DIC.record"
                               v-model="activeObj.record"></avue-select>
                </el-form-item>
              </template>
              <template v-else-if="isPublic">
                <el-form-item label="公共数据集选择">
                  <avue-select :dic="DIC.public"
                               v-model="activeObj.public"></avue-select>
                </el-form-item>
              </template>
              <template v-else-if="isMqtt">
                <el-form-item label="MQTT地址">
                  <el-input v-model="activeObj.mqttUrl">
                  </el-input>
                </el-form-item>
                <el-form-item label="MQTT配置">
                  <monaco-editor v-model="activeObj.mqttConfig"
                                 height="250"></monaco-editor>
                </el-form-item>
              </template>
              <el-form-item label="刷新时间"
                            v-if="!isStatic&&!isPublic">
                <el-input v-model="activeObj.time"
                          placeholder="0">
                  <template #append>
                    <span>毫秒</span>
                  </template>
              ...
              <el-form-item label="过滤器">
                <div class="avue-flex">
                  <avue-select v-model="activeObj.dataFormatterId"
                               :dic="filterDic"
                               style="width:100px;"
                               :props="{label:'name',value:'id'}"></avue-select>
                  <el-button v-if="!activeObj.dataFormatterId"
                             type="primary"
                             icon="el-icon-edit"
            ...
            <el-form label-width="90px"
              ...
              <el-form-item label="分组">
                <div class="avue-flex">
                  ...
                  &nbsp;
                  <el-button type="primary"
                             @click="handleMoveGroup">移动</el-button>
                </div>
              </el-form-item>
              <options v-if="!isFolder"></options>
              <el-form-item label=""
                            label-width="0px">
                <monaco-editor v-model="activeObj"
                               disabled
                               @click="handleCopy(JSON.stringify(activeObj,null,4))"
                               language="javascript"
                               height="100"></monaco-editor>
              </el-form-item>
            </el-form>
...
import glob from '@/page/setup/glob.vue'
import event from '@/page/setup/event.vue'
import transfer from '@/page/setup/transfer.vue'
import database from '@/page/setup/database.vue'
import headers from './group/header.vue';
import sync from '@/page/setup/sync.vue'
import options from '@/page/setup/options.vue'
import imglist from '@/page/setup/imglist.vue'
import group from '@/page/setup/group.vue'
import theme from '@/page/setup/theme.vue'
import filters from '@/page/setup/filters.vue'
import variable from '@/page/setup/variable.vue'
import anima from '@/page/setup/anima.vue'
...
export default {
  ...
  computed: {
    ...
    isNode () {
      return this.activeObj.dataType == 7
    },
    isFolder () {

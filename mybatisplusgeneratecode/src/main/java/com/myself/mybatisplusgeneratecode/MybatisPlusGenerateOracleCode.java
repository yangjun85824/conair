...
public class MybatisPlusGenerateOracleCode {
    ...
    public static void main(String[] args) {
        ...
        gc.setAuthor("YANG JIAN JUN");//文件创建注释
        ...
        dsc.setUrl("jdbc:mysql://172.19.3.24:3306/be_trade?allowMultiQueries=true&useUnicode=true&characterEncoding=utf-8");
        dsc.setDbType(DbType.MYSQL);
//        dsc.setTypeConvert(new OracleTypeConvert());
        dsc.setTypeConvert(new MySqlTypeConvert());
        dsc.setDriverName("com.mysql.cj.jdbc.Driver");
        dsc.setUsername("be_trade");
        dsc.setPassword("be_trade#2o220223");
        mpg.setDataSource(dsc);

        // 包配置
        PackageConfig pc = new PackageConfig();
        pc.setModuleName(scanner("模块名"));//指定生成到具体的模块名
        pc.setParent("com.sip");//指定生成的父路径
...
//        strategy.setInclude(new String[]{"",""});//需要生成的表名，多个表可以用英文逗号分开
        strategy.setTablePrefix(pc.getModuleName() + "y");//生成时是否以模块名为前缀，默认模块名为前缀
//        strategy.setTablePrefix("y_");
        String tables[] = scanner("表名，多个英文逗号分割").split(",");
        strategy.setInclude(tables);//需要生成的表名，多个表可以用英文逗号分开
